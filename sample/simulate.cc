...
double pd_control(double kp, double kd, double ff,
                double p_des, double dp_des,
                double p_cur, double dp_cur) {
    double out;
    out  = kp*(p_des - p_cur) + kd*(dp_des - dp_cur) + ff;
    return out;
}
    ...
    if (m) {
      ...
      if (settings.run) {
        ...
        if( d->time*settings.slow_down<simsync || tmstart<cpusync || cpusync==0 ||
          ...
          double joint1_force = pd_control(200, 10, 0, 0, 0,d->qpos[0], d->qvel[0] );
          d->ctrl[0] = joint1_force;
          mj_step(m, d);
        ...
        // in-sync
          ...
          while ((d->time*settings.slow_down-simsync) < (glfwGetTime()-cpusync) &&
            ...
            // clear old perturbations, apply new
            mju_zero(d->xfrc_applied, 1*m->nbody);
            mjv_applyPerturbPose(m, d, &pert, 0);  // move mocap bodies only
            mjv_applyPerturbForce(m, d, &pert);
            ...
            if(step == 0) {
                ...
                double joint1_force = pd_control(200, 10, 0, -0.03, 0,d->qpos[0], d->qvel[0] );
                d->ctrl[0] = joint1_force;
                std::cout << "joint1_force: " << joint1_force << std::endl;

                if(std::abs(d->qpos[0] + 0.03) < 0.005) {
                    cnt++;
                }
                else {
                    cnt = 0;
...
int main(int argc, const char** argv) {
  ...
  // if (argc>1) {
    ...
    mju::strcpy_arr(filename,
                    "/home/huangyong/CLionProjects/force_control_sim/model/force_control/force_control.xml");
